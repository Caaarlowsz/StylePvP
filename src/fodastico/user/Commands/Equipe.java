...
public class Equipe implements CommandExecutor {
	public boolean onCommand(final CommandSender Sender, final Command Cmd, final String Label, final String[] Args) {
		...
		if (Args.length != 0
				|| (!Cmd.getName().equalsIgnoreCase("equipe") && !Cmd.getName().equalsIgnoreCase("staffonline"))) {
			return false;
		}
		if (!p.hasPermission("kitpvp.equipe")) {
			p.sendMessage(
					"§b§lSTAFFS ONLINE §fVoc\u00ea n\u00e3o possui §c§lPERMISSAO §fpara ver os §9§lSTAFFERS ONLINE§f!");
			return true;
		}
