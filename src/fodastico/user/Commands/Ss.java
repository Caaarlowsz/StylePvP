...
public class Ss implements CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("ss")) {
            ...
            if (args.length == 0) {
                p.sendMessage("§4§lSCREENSHARE §fUtilize: /ss <jogador> - §3§lCOLOCA §fou §a§lLIBERA §fum jogador da §c§lSCREENSHARE");
                p.sendMessage("§4§lSCREENSHARE §fUtilize: /ss <check> - Veja a §2§lLISTA §fde jogadores em §c§lSCREENSHARE");
                return true;
            }
            ...
            if (!Ss.inss.contains(ip)) {
                ...
                p.sendMessage("§4§lSCREENSHARE §fVoc\u00ea §e§lCOLOCOU §fo jogador §1§l" + ip.getName() + " §fem §c§lSCREENSHARE");
                ip.sendMessage("§4§lSCREENSHARE §fVoc\u00ea foi §e§lCOLOCADO §fem §c§lSCREENSHARE");
                Bukkit.broadcastMessage("§4§lSCREENSHARE §fO jogador §1§l" + ip.getName() + " §ffoi §e§lCOLOCADO §fem §c§lSCREENSHARE");
                APIs.Ss(ip);
                WarpsManager.Ir(ip, "Ss");
                WarpsManager.Ir(p, "Ss");
                p.showPlayer(ip);
            ...
            Main.sc.registerScoreBoard(ip);
            Main.sc.registerScoreBoard(p);
            p.sendMessage("§4§lSCREENSHARE §fVoc\u00ea §a§lLIBEROU §fo jogador §1§l" + ip.getName() + " §fda §c§lSCREENSHARE");
            ip.sendMessage("§4§lSCREENSHARE §fVoc\u00ea foi §a§lLIBERADO §fda §c§lSCREENSHARE");
            Bukkit.broadcastMessage("§4§lSCREENSHARE §fO jogador §1§l" + ip.getName() + " §ffoi §a§lLIBERADO §fda §c§lSCREENSHARE");
            APIs.Normal(ip);
            p.showPlayer(ip);
            ip.showPlayer(p);
            p.showPlayer(ip);
