	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("tp")) {
			if (!p.hasPermission("kitpvp.tp")) {
				p.sendMessage(
						"§e§lPERMISSAO §fVoc\u00ea n\u00e3o possui §c§lPERMISSAO§f para executar este §b§lCOMANDO§f!");
				return true;
			}
			if (args.length == 0) {
			...
			if (args.length > 1) {
				p.sendMessage(
						"§6§lTELEPORT §fUtilize apenas: /tp <jogador>, o §6§lTELEPORT §fpara coordenadas n\u00e3o est\u00e1 §c§lDISPONIVEL§f ainda.");
				return true;
			}
		}
		return false;
