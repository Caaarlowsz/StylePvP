	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		if (cmd.getName().equalsIgnoreCase("vipset")) {
			if (!sender.hasPermission("kitpvp.vipset")) {
				sender.sendMessage(
						"§e§lPERMISSAO §7Voc\u00ea n\u00e3o possui §4§lPERMISSAO §7para executar este §3§lCOMANDO.");
				return true;
			}
			if (args.length == 0) {
			...
			if (vip.equalsIgnoreCase("light")) {
				sender.sendMessage("§6§lVIPSET §fVoc\u00ea setou o §6§lVIP §a§lLIGHT §fna conta do jogador jogador §e"
						+ player.getName() + "§e(" + player.getUniqueId() + "§e)");
			...
			if (vip.equalsIgnoreCase("premium")) {
				sender.sendMessage("§6§lVIPSET §fVoc\u00ea setou o §6§lVIP §6§lPREMIUM §fna conta do jogador jogador §e"
						+ player.getName() + "§e(" + player.getUniqueId() + "§e)");
				Bukkit.dispatchCommand((CommandSender) Bukkit.getConsoleSender(),
			...
			if (vip.equalsIgnoreCase("beta")) {
				sender.sendMessage("§6§lVIPSET §fVoc\u00ea setou o §6§lVIP §1§lBETA §fna conta do jogador jogador §e"
						+ player.getName() + "§e(" + player.getUniqueId() + "§e)");
				Bukkit.dispatchCommand((CommandSender) Bukkit.getConsoleSender(),
						"pex user " + player.getName() + " group set beta");
				TituloAPI.MandarTitulo(player, "§6§lVIP");
			...
			if (vip.equalsIgnoreCase("ultimate")) {
				sender.sendMessage(
						"§6§lVIPSET §fVoc\u00ea setou o §6§lVIP §d§lULTIMATE §fna conta do jogador jogador §e"
								+ player.getName() + "§e(" + player.getUniqueId() + "§e)");
				Bukkit.dispatchCommand((CommandSender) Bukkit.getConsoleSender(),
