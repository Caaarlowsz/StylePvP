    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String commandLabel, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("sc")) {
            ...
            if (inStaffChat(p)) {
                ...
                Bukkit.broadcast("§b§lSTAFF-CHAT " + p.getDisplayName() + " §c§lSAIU §fdo §9§lSTAFF-CHAT§f!", "kitpvp.sc");
            }
            else {
                Sc.inStaffChat.add(p.getName());
                Bukkit.broadcast("§b§lSTAFF-CHAT " + p.getDisplayName() + " §a§lENTROU §fno §9§lSTAFF-CHAT§f!", "kitpvp.sc");
            }
        }
        return true;
    }
