...
public class CooldownAPI {
	...
	public static boolean checkCooldown(final Player p) {
		return !CooldownAPI.pCooldown.containsKey(p.getName())
				|| CooldownAPI.pCooldown.get(p.getName()) <= System.currentTimeMillis();
	}

	public static long getCooldown(final Player p) {
		final long tempo = TimeUnit.MILLISECONDS
				.toSeconds(CooldownAPI.pCooldown.get(p.getName()) - System.currentTimeMillis());
		if (CooldownAPI.pCooldown.containsKey(p.getName())
				|| CooldownAPI.pCooldown.get(p.getName()) > System.currentTimeMillis()) {
			return tempo;
		}
	...
	public static long getCooldown(final String string) {
		final long tempo = TimeUnit.MILLISECONDS
				.toSeconds(CooldownAPI.pCooldown.get(string) - System.currentTimeMillis());
		if (CooldownAPI.pCooldown.containsKey(string)
				|| CooldownAPI.pCooldown.get(string) > System.currentTimeMillis()) {
			return tempo;
		}
		return 0L;
	}
