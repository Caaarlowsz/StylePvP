...
public class WarpsManager implements Listener
{
    ...
    public static void Set(final Player p, final String Warp) {
        ...
        WarpsConfigurations.getConfig().config().set("Warps." + Warp + ".World", (Object)p.getLocation().getWorld().getName());
    ...
    public static void Ir(final Player p, final String Warp) {
        if (WarpsConfigurations.getConfig().config().contains("Warps." + Warp)) {
            ...
            final World world = Bukkit.getWorld(WarpsConfigurations.getConfig().config().getString("Warps." + Warp + ".World"));
            final Location loc = new Location(world, x, y, z, Yaw, Pitch);
            p.teleport(loc);
        }
        else {
            p.sendMessage("§6§lWARP §fEsta §e§lWARP §fainda n\u00e3o foi §a§lSETADA§f! Avise um §d§lSTAFFER §fpara §a§lSETA-LA§f!");
        }
    }
}
