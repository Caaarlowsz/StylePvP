...
public class ScoreBoard implements Listener {
	...
	public static void UpdatePVP(final Player p) {
		if (Score.ativada.contains(p.getName()) && KitAPI.getkit(p) != "1v1" && KitAPI.getkit(p) != "Admin"
				&& KitAPI.getkit(p) != "Evento" && KitAPI.getkit(p) != "ScreenShare") {
			final Thread th = new Thread(new Runnable() {
				...
				public void run() {
					if (Score.ativada.contains(p.getName())) {
						...
						site.setPrefix(
								StylePvP.getInstance().getConfig().getString("MSITE-SCORE-PREFIX").replace("&", "�"));
						site.setSuffix(
								StylePvP.getInstance().getConfig().getString("MSITE-SCORE-SUFFIX").replace("&", "�"));
