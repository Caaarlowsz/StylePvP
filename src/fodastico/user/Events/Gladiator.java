...
public class Gladiator implements Listener
{
    ...
    public void OnGladiat0orKit(final PlayerInteractEntityEvent event) {
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && KitAPI.getkit(p) == "Gladiator" && CooldownAPI.pCooldown.containsKey(p.getName())) {
                p.sendMessage("§3§lGLADIATOR §fVoc\u00ea est\u00e1 em §c§lCOOLDOWN §fde §c§l" + CooldownAPI.getCooldown(p) + "s");
                return;
            }
            if (p.getItemInHand().getType() == Material.IRON_FENCE && KitAPI.getkit(p) == "Gladiator" && !CooldownAPI.pCooldown.containsKey(p.getName())) {
                    ...
                    for (int bX = -10; bX <= 10; ++bX) {
                        for (int bZ = -10; bZ <= 10; ++bZ) {
                            for (int bY = -1; bY <= 10; ++bY) {
                                ...
                                if (!b.isEmpty()) {
                                    event.setCancelled(true);
                                    p.sendMessage("§3§lGLADIATOR §fVoc\u00ea n\u00e3o pode puxar para o §9§lGLADIATOR §fneste local.");
                                    return;
                                }
                                if (bY == 10) {
                    ...
                    Gladiator.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                        ...
                        public void run() {
                            if (Gladiator.fighting.containsKey(p.getName()) && Gladiator.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && Gladiator.fighting.containsKey(r.getName()) && Gladiator.fighting.get(r.getName()).equalsIgnoreCase(p.getName())) {
                                ...
                                p.updateInventory();
                                r.updateInventory();
                                p.sendMessage("§4§lGLADIATOR §fN\u00e3o houve §a§lVENCEDORES§f, por isso voc\u00ea foi §6§lTELEPORTADO §fpara o local de origem.");
                                r.sendMessage("§4§lGLADIATOR §fN\u00e3o houve §a§lVENCEDORES§f, por isso voc\u00ea foi §6§lTELEPORTADO §fpara o local de origem.");
