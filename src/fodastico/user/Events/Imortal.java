...
public class Imortal implements Listener
{
    ...
    public void AtivarImortal(final PlayerInteractEvent e) {
        ...
        if (KitAPI.getkit(p).equalsIgnoreCase("Imortal") && (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && p.getItemInHand().getType() == Material.BEDROCK) {
            ...
            if (KitAPI.getkit(p).equalsIgnoreCase("Imortal") && (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && p.getItemInHand().getType() == Material.BEDROCK) {
                if (CooldownAPI.pCooldown.containsKey(p.getName())) {
                    p.sendMessage("§3§lIMORTAL §fVoc\u00ea est\u00e1 em §c§lCOOLDOWN §fde §c§l" + CooldownAPI.getCooldown(p) + "s");
                    return;
                }
                Imortal.regen.add(p);
