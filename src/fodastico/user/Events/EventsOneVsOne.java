...
public class EventsOneVsOne implements Listener
{
    ...
    public void PlayerInteractEntityEvent4(final PlayerInteractEntityEvent event) {
        ...
        if (Fast1v1.inSecondsWaiting.contains(p)) {
            if (p.getItemInHand().equals((Object)Get1v1Itens.Blaze)) {
                p.sendMessage("§b§l1v1 §fVoc\u00ea n\u00e3o pode §c§lDESAFIAR §fum jogador, enquanto estiver na fila para §b§l1v1 §fr\u00e1pido.");
            }
        }
            ...
            Player2.sendMessage("§b§l1v1 §fVoc\u00ea foi §c§lDESAFIADO §fpelo jogador §a§l" + p.getName() + " §fpara §b§l1v1");
    ...
    public void Morrer2(final PlayerDeathEvent e) {
        ...
        if (e.getEntity().getKiller() instanceof Player) {
            ...
            if (m != v) {
                ...
                m.sendMessage("     ");
                m.sendMessage("§a§lVITORIA §fVoc\u00ea §2§lVENCEU §fa batalha contra o §a" + v.getName() + " §fcom " + amountm + " §fsopas e " + coraexato(m) + " §fcora\u00e7\u00f5es.");
                EventsOneVsOne.inBattle.remove(m);
                EventsOneVsOne.inBattle.remove(v);
                KitAPI.setKit(v, "1v1");
                KitAPI.setKit(m, "1v1");
                v.sendMessage("     ");
                v.sendMessage("§c§lDERROTA §fVoc\u00ea §c§lPERDEU §fa batalha contra §c" + m.getName() + " §fque ficou com " + amountm + " §fsopas e " + coraexato(m) + " §fcora\u00e7\u00f5es.");
    ...
    public void onPlayerLeft(final PlayerQuitEvent e) {
        ...
        if (EventsOneVsOne.LogOutOnevsOne.containsKey(p.getName())) {
            ...
            t.sendMessage("§a§lVIT\u00d3RIA §f" + p.getName() + " §c§lDESLOGOU §fportando voc\u00ea §a§lVENCEU §fa batalha.");
    ...
    public void onPlayerLeft2(final PlayerQuitEvent e) {
        ...
        if (EventsOneVsOne.LogOutOnevsOne.containsKey(p.getName())) {
            ...
            ImortalAPI.setInvensibility(t);
            t.sendMessage("§a§lVIT\u00d3RIA §f" + p.getName() + " §c§l§oDESLOGOU §fportando voc\u00ea §2§lVENCEU §fa batalha.");
            t.sendMessage("§6§lMOEDAS §fVoc\u00ea §a§lRECEBEU §6§l50 MOEDAS");
            XpbyXpAPI.xpbyxp(p, t);
            MoneyAPI.addMoney(t, 50);
            TopKsAPI.TopKs.set(p.getName(), (Object)0);
    ...
    public void cmda(final PlayerCommandPreprocessEvent event) {
        ...
        if (EventsOneVsOne.isPlaying.contains(p)) {
            event.getPlayer().sendMessage("§b§l1v1 §fVoc\u00ea n\u00e3o pode executar §3§lCOMANDOS §fenquanto estiver §c§lLUTANDO");
